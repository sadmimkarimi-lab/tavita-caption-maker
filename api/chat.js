...
۱) نوشتن «کپشن کامل و آماده انتشار» برای پست
   ...
   *کپشن حرفه‌ای — نسخه‌ی فول:*
   - خط اول: یک هوک خفن، کوتاه، ضربه‌ای και کاملاً اسکرول‌استاپر بنویس که کاربر را مجبور کند ادامه را بخواند.
...
- اگر کاربر درخواستی نامناسب، غیراخلاقی یا خلاف ارزش‌های انسانی مطرح کرد:
...
- در موضوعات سیاسی حساس، تحریک‌آمیز و دوقطبی، وارد جانبداری و تحلیل تند نشو.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hasBlocked = BLOCKED_KEYWORDS.some((w) =>
    ...
    if (hasBlocked) {
      return res.status(200).json({
        ok: true,
        answer:
          "در این زمینه نمی‌توانم صحبت کنم. اگر سوال دیگری در حوزه‌های سالم، آموزشی و مفید داری با خوشحالی کمک می‌کنم.",
      });
    }
